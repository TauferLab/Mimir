  ...
  public:
        ...
        : BaseDatabase<KeyType, ValType>(true)
    {
        ...
        if (std::is_pointer<KeyType>::value
            ...
            bin_unit_size = (MEMPAGE_SIZE + record_size - 1) / MEMPAGE_SIZE
                            * MEMPAGE_SIZE;
            bin_per_page = (int) (DATA_PAGE_SIZE / bin_unit_size);
        }
    ...
    {
        LOG_PRINT(DBG_DATA,
                  "BinContainer close (buffer size=%ld, data size=%ld).\n",
    ...
    virtual int read(KeyType *key, ValType *val)
    {
        ...
        int kvsize = this->ser->kv_from_bytes(key, val, ptr,
                                              bin_unit_size - cur_bin_off);
    ...
    virtual int write(KeyType *key, ValType *val)
    {
        ...
        int kvsize = this->ser->kv_to_bytes(
            key, val, ptr, bin_unit_size - bins[bidx].datasize);
        if (kvsize == -1) {
            bidx = get_empty_bin();
            bins[bidx].bintag = bid;
            bin_insert_idx[bid] = bidx;
                ...
                while (src_off < bins[i].datasize) {
                    ...
                    if (iter != slices.end()) {
                        ...
                        int kvsize = this->ser->kv_from_bytes(
                            &key, &val, tmp_buf, bins[i].datasize - src_off);
                        if (src_off != dst_off) {
