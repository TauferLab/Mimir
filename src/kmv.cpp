  ...
  for (int i = 0; i< mr->svec_spillf_kv_after_all2all.size(); i++)
  {
    ...
    if (tid == 0)
    {
      ...
      if (kv_file.is_open()){
        ...
        if (kv_file_size == 0) continue;//skip empty files
      ...
      for (auto u = ul->begin(); u != ul->end(); ++u)
      {
        ...
        if (u->key != NULL)
        {
          ...
          if ((mv_convert_offset + u->mv_size)<MVC_SIZE){
            ...
            num_spill_mv_c++;
            spill_mv_c_file = mr->spill_base + "/spill_mv_c.p" + std::to_string(me)+              "t"+std::to_string(tid)+"."+
              std::to_string(num_spill_mv_c)+"."+ std::to_string(reduce_num);
