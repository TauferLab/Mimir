...
#define DBG_GEN 1    // general information
#define DBG_DATA 2   // intermediate data management
#define DBG_COMM 4   // communication
#define DBG_IO 8     // disk io
#define DBG_MEM 16   // memory management
#define DBG_CHUNK 32 // input chunks
...
#define LOG_PRINT(type, ...)                                     \
    do {                                                         \
        ...
        if (DBG_LEVEL & (type)) {                                \
            ...
            fprintf(stdout, "%d[%d] %s:%d %s", mimir_world_rank, \
                    mimir_world_size, __FILE__, __LINE__, _s);   \
            fflush(stdout);                                      \
        }                                                        \
    } while (0)
#else
...
#define LOG_WARNING(...)                                                       \
    do {                                                                       \
        char _s[MIMIR_MAX_LOG_LEN] = "";                                       \
        sprintf(_s, __VA_ARGS__);                                              \
        fprintf(stderr, "%d[%d] %s:%d %s", mimir_world_rank, mimir_world_size, \
