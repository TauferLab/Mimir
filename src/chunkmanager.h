...
{
  public:
    ChunkManager(MPI_Comm comm, std::vector<std::string> input_dir,
                 SplitPolicy policy = BYNAME)
    {
        ...
        FileSplitter::getFileSplitter(chunk_mgr_comm)
            ->split(&filelist, file_list, policy);
        file_list[chunk_mgr_rank].print();
        // get file chunk number
        this->total_chunk = 0;
        this->chunk_nums = (int*) mem_aligned_malloc(
            MEMPAGE_SIZE, sizeof(int) * chunk_mgr_size);
    ...
    virtual int send_head(Chunk& chunk, char* buffer, int bufsize)
    {
        ...
        if (msg_list[msg_idx].target_rank != PROC_RANK_PENDING) {
            LOG_PRINT(DBG_CHUNK,
                      "Chunk: send head (%d) of chunk <%d,%d> to %d\n", bufsize,
                      chunk.procrank, chunk.localid,
                      msg_list[msg_idx].target_rank);
    ...
    virtual int recv_tail(Chunk& chunk, char* buffer, int bufsize)
    {
        ...
        MPI_Irecv(buffer, bufsize, MPI_BYTE, next_rank, CHUNK_TAIL_TAG,
                  chunk_mgr_comm, &req);
        while (!flag) {
        ...
        LOG_PRINT(DBG_CHUNK,
                  "Chunk: recv tail (length=%d) of chunk <%d,%d> from %d\n",
    ...
    virtual void make_progress()
    {
        for (size_t i = 0; i < msg_list.size(); i++) {
            ...
            if (msg_list[i].msg_state == MsgPending) {
                ...
                msg_list[i].target_rank
                    = prev_chunk_worker(msg_list[i].msg_chunk);
  ...
  protected:
    ...
    {
        if (chunk.procrank > 0 && chunk.localid == 0) return chunk.procrank - 1;
        ...
        for (size_t i = 0; i < filesegs.size(); i++) {
            ...
            if (total_chunk + file_chunk > chunk_id) {
                chunk.fileoff = (chunk_id - total_chunk) * INPUT_BUF_SIZE
                                + filesegs[i].startpos;
                if (filesegs[i].filesize - chunk.fileoff
                ...
                LOG_PRINT(DBG_CHUNK,
                          "Chunk: get chunk <%d,%d> from %d (fileoff=%ld, "
                          "chunksize=%ld)\n",
                          chunk.procrank, chunk.localid, chunk.procrank,
                          chunk.fileoff, chunk.chunksize);
                return true;
            }
...
{
  public:
    StealChunkManager(MPI_Comm comm, std::vector<std::string> input_dir,
                      SplitPolicy policy = BYNAME)
        : ChunkManager<KeyType, ValType>(comm, input_dir, policy)
    {
        steal_off = 0;
        chunk_map = (int*) mem_aligned_malloc(
            MEMPAGE_SIZE, sizeof(int) * this->chunk_nums[this->chunk_mgr_rank]);
        ...
        MPI_Win_create(&steal_off, sizeof(int), sizeof(int), MPI_INFO_NULL,
                       this->chunk_mgr_comm, &steal_off_win);
        MPI_Win_create(
            chunk_map, sizeof(int) * this->chunk_nums[this->chunk_mgr_rank],
    ...
    virtual bool acquire_chunk(Chunk& chunk)
    {
        ...
        MPI_Fetch_and_op(&one, &my_chunk_id, MPI_INT, this->chunk_mgr_rank, 0,
                         MPI_SUM, chunk_id_win);
#else
        MPI_Get_accumulate(&one, 1, MPI_INT, &my_chunk_id, 1, MPI_INT,
                           this->chunk_mgr_rank, 0, 1, MPI_INT, MPI_SUM,
                           chunk_id_win);
#endif
    ...
    virtual bool acquire_local_chunk(Chunk& chunk, int localid)
    {
        ...
        MPI_Win_unlock(this->chunk_mgr_rank, chunk_id_win);
        if (ret_idx == localid) {
            MPI_Win_lock(MPI_LOCK_SHARED, this->chunk_mgr_rank, 0,
                         chunk_map_win);
            MPI_Accumulate(&(this->chunk_mgr_rank), 1, MPI_INT,
                           this->chunk_mgr_rank, localid, 1, MPI_INT,
                           MPI_REPLACE, chunk_map_win);
            MPI_Win_unlock(this->chunk_mgr_rank, chunk_map_win);
  ...
  protected:
    virtual bool steal_chunk(Chunk& chunk)
    {
        ...
        while (steal_off < this->chunk_mgr_size) {
            int victim_rank
                = (this->chunk_mgr_rank + steal_off) % this->chunk_mgr_size;
            int victim_steal_off = 0;
            MPI_Win_lock(MPI_LOCK_SHARED, victim_rank, 0, steal_off_win);
            int tmp;
#ifdef MPI_FETCH_AND_OP
            MPI_Fetch_and_op(&tmp, &victim_steal_off, MPI_INT, victim_rank, 0,
                             MPI_NO_OP, steal_off_win);
#else
            MPI_Get_accumulate(&tmp, 1, MPI_INT, &victim_steal_off, 1, MPI_INT,
                               victim_rank, 0, 1, MPI_INT, MPI_NO_OP,
                               steal_off_win);
#endif
            MPI_Win_unlock(victim_rank, steal_off_win);
            LOG_PRINT(DBG_CHUNK,
                      "Chunk: try to steal from %d (steal offset=%d)\n",
                      victim_rank, victim_steal_off);
            if (victim_steal_off == 0) {
                MPI_Win_lock(MPI_LOCK_SHARED, victim_rank, 0, chunk_id_win);
#ifdef MPI_FETCH_AND_OP
                MPI_Fetch_and_op(&one, &local_chunk_id, MPI_INT, victim_rank, 0,
                                 MPI_SUM, chunk_id_win);
#else
                MPI_Get_accumulate(&one, 1, MPI_INT, &local_chunk_id, 1,
                                   MPI_INT, victim_rank, 0, 1, MPI_INT, MPI_SUM,
                                   chunk_id_win);
#endif
                ...
                if (local_chunk_id < this->chunk_nums[victim_rank]) {
                    ...
                    MPI_Win_lock(MPI_LOCK_SHARED, victim_rank, 0,
                                 chunk_map_win);
                    MPI_Accumulate(&(this->chunk_mgr_rank), 1, MPI_INT,
                                   victim_rank, local_chunk_id, 1, MPI_INT,
                                   MPI_REPLACE, chunk_map_win);
    ...
    int get_chunk_worker(int64_t globalid)
    {
        ...
        int tmp;
#ifdef MPI_FETCH_AND_OP
        MPI_Fetch_and_op(&tmp, &worker_rank, MPI_INT, chunk_owner_rank,
                         chunk_owner_id, MPI_NO_OP, chunk_map_win);
#else
    ...
    {
        for (int i = 0; i < this->chunk_nums[this->chunk_mgr_rank]; i++) {
            printf("%d[%d] %d:%d\n", this->chunk_mgr_rank, this->chunk_mgr_size,
                   i, chunk_map[i]);
        }
    }
