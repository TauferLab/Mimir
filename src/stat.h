...
#define EVENT_DISK_MPIREADATALL \
    "event_disk_mpireadatall" // MPI_File_read_at_all
#define EVENT_DISK_MPIWRITEATALL \
    "event_disk_mpiwriteatall"                    // MPI_File_write_at_all
...
#define GET_CUR_TIME                                                  \
    {                                                                 \
        if (mimir_world_rank == 0) {                                  \
            ...
            sprintf(timestr, "%d-%d-%d-%d-%d-%d", tm.tm_year + 1900,  \
...
#define PROFILER_RECORD_TIME_START profiler_info.prev_wtime = MR_GET_WTIME();

#define PROFILER_RECORD_TIME_END(timer_type) \
    profiler_timer[timer_type] += (MR_GET_WTIME() - profiler_info.prev_wtime);
    ...
    {                                                                          \
        ...
        if (mimir_world_rank == 0) {                                           \
            ...
            for (int i = 0; i < TIMER_NUM; i++)                                \
                fprintf(fp, ",%s", timer_str[i]);                              \
            for (int i = 0; i < COUNTER_NUM; i++)                              \
                fprintf(fp, ",%s", counter_str[i]);                            \
        ...
        if (mimir_world_rank == 0) {                                           \
            ...
            for (int i = 1; i < mimir_world_size; i++) {                       \
                ...
                MPI_Recv(profiler_timer, TIMER_NUM, MPI_DOUBLE, i,             \
                         STAT_TIMER_TAG, mimir_world_comm, &st);               \
                for (int i = 0; i < TIMER_NUM; i++)                            \
                    fprintf(fp, ",%g", profiler_timer[i]);                     \
                MPI_Recv(profiler_counter, COUNTER_NUM, MPI_UINT64_T, i,       \
                         STAT_COUNTER_TAG, mimir_world_comm, &st);             \
                for (int i = 0; i < COUNTER_NUM; i++)                          \
                    fprintf(fp, ",%ld", profiler_counter[i]);                  \
            }                                                                  \
        }                                                                      \
        else {                                                                 \
            MPI_Send(profiler_timer, TIMER_NUM, MPI_DOUBLE, 0, STAT_TIMER_TAG, \
                     mimir_world_comm);                                        \
            MPI_Send(profiler_counter, COUNTER_NUM, MPI_UINT64_T, 0,           \
                     STAT_COUNTER_TAG, mimir_world_comm);                      \
        }                                                                      \
...
#define TRACKER_START                                                 \
    {                                                                 \
        if (mimir_world_rank == 0) {                                  \
            tracker_event = new std::vector<                          \
                std::pair<std::string, double>>[mimir_world_size];    \
        }                                                             \
        else {                                                        \
            tracker_event                                             \
                = new std::vector<std::pair<std::string, double>>[1]; \
        }                                                             \
...
#define TRACKER_RECORD_EVENT(event_type)                       \
    {                                                          \
        if (OUTPUT_TRACE) {                                    \
            ...
            tracker_event[0].push_back(                        \
                std::make_pair(event_type, t_start - t_prev)); \
...
#define TRACKER_PRINT(filename)                                                \
    {                                                                          \
        ...
        for (iter = tracker_event[0].begin(); iter != tracker_event[0].end();  \
             iter++) {                                                         \
            max_bytes += (int) strlen(iter->first.c_str()) + 1;                \
            max_bytes += (int) sizeof(iter->second);                           \
        }                                                                      \
        MPI_Reduce(&max_bytes, &total_bytes, 1, MPI_INT, MPI_MAX, 0,           \
                   mimir_world_comm);                                          \
        ...
        if (mimir_world_rank == 0) {                                           \
            ...
            for (int i = 0; i < mimir_world_size - 1; i++) {                   \
                MPI_Recv(tmp, total_bytes, MPI_BYTE, MPI_ANY_SOURCE,           \
                         STAT_EVENT_TAG, mimir_world_comm, &st);               \
                ...
                while (off < recv_count) {                                     \
                    ...
                    tracker_event[recv_rank].push_back(                        \
            ...
            int off = 0;                                                       \
            std::vector<std::pair<std::string, double>>::iterator iter;        \
            for (iter = tracker_event[0].begin();                              \
                 iter != tracker_event[0].end(); iter++) {                     \
                memcpy(tmp + off, iter->first.c_str(),                         \
                       strlen(iter->first.c_str()) + 1);                       \
