                           ...
                           public Combinable<KeyType, ValType>
{
  public:
    CombineKVContainer(
        ...
        void *user_ptr,
        //uint32_t bincount = 0,
        int keycount = 1, int valcount = 1, int hashscale = 1)
    ...
    virtual int write(KeyType *key, ValType *val)
    {
        ...
        if (u == NULL) {
            ...
            if (!(this->ispointer)) {
                ...
                if (iter == this->slices.end()) {
                    if (this->pageid >= this->pages.size()
                        || this->pagesize - this->pages[this->pageid].datasize
                               < kvsize) {
                        this->pageid = this->add_page();
                    }

                    tmp.kv = this->pages[this->pageid].buffer
                             + this->pages[this->pageid].datasize;
                    this->ser->kv_to_bytes(key, val, tmp.kv, kvsize);
            ...
            if (rvalsize <= uvalsize) {
                ...
                if (ukeysize + rvalsize + this->pages[this->pageid].datasize
                    > this->pagesize)
                    this->pageid = this->add_page();
                u->kv = this->pages[this->pageid].buffer
                        + this->pages[this->pageid].datasize;
                this->ser->kv_to_bytes(u_key, r_val, u->kv,
                                       ukeysize + rvalsize);
    ...
    virtual void garbage_collection()
    {
        ...
        if (!(this->slices.empty())) {
            LOG_PRINT(DBG_GEN,
                      "CombineKVContainer garbage collection: slices=%ld\n",
                      this->slices.size());
                ...
                while (src_off < src_page->datasize) {
                    ...
                    if (slice != this->slices.end()) {
                        ...
                        if (dst_page != src_page || dst_off != src_off) {
                            ...
                            int keysize = this->ser->get_key_bytes(key);
                            char *keyptr = this->ser->get_key_ptr(key);
                            u = bucket->updateEntry(keyptr, keysize,
                                                    dst_page->buffer + dst_off);
