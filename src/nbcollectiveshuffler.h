...
enum ShuffleMsgState { ShuffleMsgStart, ShuffleMsgPending, ShuffleMsgComplete };
...
{
  public:
    NBCollectiveShuffler(MPI_Comm comm, Writable<KeyType, ValType> *out,
                         int (*user_hash)(KeyType *key, ValType *val,
                                          int npartition),
                         int keycount, int valcount, bool split_hint,
                         HashBucket<> *h)
        : BaseShuffler<KeyType, ValType>(comm, out, user_hash, keycount,
                                         valcount, split_hint, h)
    {
        unit_size = MEMPAGE_SIZE;
        while ((int64_t) unit_size * (int64_t) this->shuffle_size
               > COMM_BUF_SIZE) {
            unit_size /= 2;
        }
        if (unit_size < 64) {
    ...
    virtual int open()
    {
        ...
        a2av_token = 0;
        pending_msg = 0;
        for (int i = 0; i < MIN_SBUF_COUNT; i++) insert_comm_buffer();

        a2a_s_count = (int *) mem_aligned_malloc(
            MEMPAGE_SIZE, sizeof(int) * this->shuffle_size);
        ;
        a2a_s_displs = (int *) mem_aligned_malloc(
            MEMPAGE_SIZE, sizeof(int) * this->shuffle_size);
        ;
        a2a_r_count = (int *) mem_aligned_malloc(
            MEMPAGE_SIZE, sizeof(int) * this->shuffle_size);
        ;
        a2a_r_displs = (int *) mem_aligned_malloc(
            MEMPAGE_SIZE, sizeof(int) * this->shuffle_size);
        ;
    ...
    virtual int write(KeyType *key, ValType *val)
    {
        ...
        char *buffer = msg_buffers[cur_idx].send_buffer
                       + target * (int64_t) buf_size
                       + msg_buffers[cur_idx].send_offset[target];
        int kvsize = this->ser->kv_to_bytes(
            key, val, buffer,
            (int) buf_size - msg_buffers[cur_idx].send_offset[target]);
        if (kvsize == -1) {
            start_kv_exchange();
            target = this->get_target_rank(key, val);
            buffer = msg_buffers[cur_idx].send_buffer
                     + target * (int64_t) buf_size
                     + msg_buffers[cur_idx].send_offset[target];
    ...
    {
        if (issue_new && pending_msg == 0) start_kv_exchange();
        push_kv_exchange();
    }
  ...
  protected:
    void wait()
    {
        ...
        for (int i = 0; i < this->shuffle_size; i++)
            msg_buffers[cur_idx].send_bytes
                += (uint64_t) msg_buffers[cur_idx].send_offset[i];

        if (msg_buffers[cur_idx].send_bytes > 0) start_kv_exchange();
    ...
    void start_kv_exchange()
    {
        if (this->done_flag) {
            ...
            for (int i = 0; i < this->shuffle_size; i++)
                msg_buffers[cur_idx].send_bytes
                    += (uint64_t) msg_buffers[cur_idx].send_offset[i];
            PROFILER_RECORD_COUNT(COUNTER_SEND_BYTES,
                                  msg_buffers[cur_idx].send_bytes, OPSUM);
        }
        PROFILER_RECORD_COUNT(COUNTER_SHUFFLE_TIMES, 1, OPSUM);

        MPI_Ialltoall(msg_buffers[cur_idx].send_offset, 1, MPI_INT,
                      msg_buffers[cur_idx].recv_count, 1, MPI_INT, a2a_comm,
                      &(msg_buffers[cur_idx].a2a_req));
        ...
        LOG_PRINT(
            DBG_COMM, "Comm: MPI_Ialltoall start. (token=%ld, send=%ld)\n",
        ...
        if (cur_idx == -1) {
            if (buf_count < MAX_SBUF_COUNT) {
                ...
                while (1) {
                    push_kv_exchange();
                    if (done_kv_exchange(cur_idx)) break;
        ...
        for (int k = 0; k < buf_count; k++) {
                    ...
                    if (flag) {
                        ...
                        for (int i = 0; i < this->shuffle_size; i++) {
                            if (msg_buffers[k].recv_count[i] == -1)
                                ...
                                msg_buffers[k].recv_bytes
                                    += (int64_t) msg_buffers[k].recv_count[i];
                        }
                        LOG_PRINT(DBG_COMM,
                                  "Comm: MPI_Ialltoall finish (token=%ld, "
                                  "done=%d).\n",
                ...
                if (msg_buffers[k].msg_token == a2av_token
                    ...
                    for (int i = 0; i < this->shuffle_size; i++) {
                        ...
                        a2a_s_count[i] = (msg_buffers[k].send_offset[i]
                                          + (0x1 << type_log_bytes) - 1)
                                         >> type_log_bytes;
                        a2a_r_count[i] = (msg_buffers[k].recv_count[i]
                                          + (0x1 << type_log_bytes) - 1)
                                         >> type_log_bytes;
                        a2a_s_displs[i]
                            = (i * (int) buf_size) >> type_log_bytes;
                    }
                    a2a_r_displs[0] = 0;
                    for (int i = 1; i < this->shuffle_size; i++)
                        a2a_r_displs[i]
                            = a2a_r_displs[i - 1] + a2a_r_count[i - 1];

                    LOG_PRINT(DBG_COMM,
                              "Comm: MPI_Ialltoallv start "
                              "(token=%ld,send=%ld,recv=%ld).\n",
                              msg_buffers[k].msg_token,
                              msg_buffers[k].send_bytes,
                              msg_buffers[k].recv_bytes);

                    MPI_Ialltoallv(msg_buffers[k].send_buffer, a2a_s_count,
                                   a2a_s_displs, comm_type,
                                   msg_buffers[k].recv_buffer, a2a_r_count,
                                   a2a_r_displs, comm_type, a2av_comm,
                                   &(msg_buffers[k].a2av_req));

                    PROFILER_RECORD_COUNT(COUNTER_RECV_BYTES,
                                          (uint64_t) msg_buffers[k].recv_bytes,
                                          OPSUM);
            ...
            if (msg_buffers[k].msg_state == ShuffleMsgPending) {
                if (msg_buffers[k].a2av_req != MPI_REQUEST_NULL) {
                    ...
                    if (flag) {
                        LOG_PRINT(DBG_COMM,
                                  "Comm: MPI_Ialltoallv finish (token=%ld, "
                                  "done=%d).\n",
                                  msg_buffers[k].msg_token, this->done_count);
    ...
    void wait_all()
    {
        while (pending_msg > 0) push_kv_exchange();
    }
        ...
        for (k = 0; k < this->shuffle_size; k++) {
            ...
            while (count < msg_buffers[idx].recv_count[k]) {
                ...
                int kvsize = this->ser->kv_from_bytes(
                    &key, &val, src_buf,
                    this->msg_buffers[idx].recv_count[k] - count);
                this->out->write(key, val);
                src_buf += kvsize;
                count += kvsize;
            }
            int padding = msg_buffers[idx].recv_count[k]
                          & ((0x1 << type_log_bytes) - 0x1);
            src_buf += padding;
        }
    }
    ...
    {
        ShuffleMsgBuf buf;
        buf.send_buffer = (char *) mem_aligned_malloc(
            MEMPAGE_SIZE, buf_size * this->shuffle_size);
        buf.recv_buffer = (char *) mem_aligned_malloc(
            MEMPAGE_SIZE, buf_size * this->shuffle_size);
        buf.send_offset = (int *) mem_aligned_malloc(
            MEMPAGE_SIZE, sizeof(int) * this->shuffle_size);
        buf.recv_count = (int *) mem_aligned_malloc(
            MEMPAGE_SIZE, sizeof(int) * this->shuffle_size);
