...
{
  public:
    CollectiveShuffler(MPI_Comm comm, Writable<KeyType, ValType> *out,
    ...
    virtual int open()
    {
        ...
        LOG_PRINT(DBG_GEN,
                  "CollectiveShuffler open: allocation start, peakmem=%ld\n",
                  peakmem);

        send_buffer = (char *) mem_aligned_malloc(
            MEMPAGE_SIZE, buf_size * this->shuffle_size, MCDRAM_ALLOCATE);
        recv_buffer = (char *) mem_aligned_malloc(
            MEMPAGE_SIZE, buf_size * this->shuffle_size, MCDRAM_ALLOCATE);
        send_offset = (int *) mem_aligned_malloc(
            MEMPAGE_SIZE, sizeof(int) * this->shuffle_size, MCDRAM_ALLOCATE);
        recv_count = (int *) mem_aligned_malloc(
            MEMPAGE_SIZE, sizeof(int) * this->shuffle_size, MCDRAM_ALLOCATE);
        ...
        LOG_PRINT(DBG_GEN,
                  "CollectiveShuffler open: buf_size=%ld, peakmem=%ld\n",
                  buf_size, peakmem);
    ...
    virtual int write(KeyType *key, ValType *val)
    {
        ...
        char *buffer
            = send_buffer + target * (int64_t) buf_size + send_offset[target];
        int kvsize = this->ser->kv_to_bytes(
            key, val, buffer, (int) buf_size - send_offset[target]);

        if (kvsize == -1) {
            TRACKER_RECORD_EVENT(EVENT_COMPUTE_MAP);
            exchange_kv();
            target = this->get_target_rank(key, val);
            buffer = send_buffer + target * (int64_t) buf_size
                     + send_offset[target];
    ...
    bool check_load_balance()
    {
        ...
        if (!(this->out_combiner)) {
            ...
            if ((double) max_unique_count
                > BALANCE_FACTOR * (double) min_unique_count)
                return false;
        }
    ...
    void gather_counts()
    {
        if (!(this->out_combiner)) {
            ...
            if (this->shared_rank == 0) {
                MPI_Allgatherv(this->kv_per_core, this->shared_size,
                               MPI_INT64_T, this->kv_per_proc,
                               this->proc_map_count, this->proc_map_off,
            ...
            if (this->shared_rank == 0) {
                MPI_Allgatherv(this->kv_per_core, this->shared_size,
                               MPI_INT64_T, this->unique_per_proc,
                               this->proc_map_count, this->proc_map_off,
                               MPI_INT64_T, this->node_comm);
            }
        }
        ...
        for (i = 0; i < this->shuffle_size; i++) {
            if (!(this->out_combiner))
                this->global_kv_count += this->kv_per_proc[i];
            else
                this->global_unique_count += this->unique_per_proc[i];
        }
    }
    ...
    void balance_load()
    {
        ...
        MPI_Allgatherv(sendbuf, sendcount, MPI_CHAR, recvbuf, recvcounts,
                       displs, MPI_CHAR, this->shuffle_comm);
    ...
    void split_keys()
    {
        ...
        while (iter != this->bin_table_flip.rend()) {
            ...
            if (iter->first * this->shuffle_size > this->global_kv_count) {
                ...
                LOG_PRINT(
                    DBG_REPAR, "Find split suspect bid=%u (%ld,%lf)\n",
        ...
        if (suspect_table.size() != 0) {
            ...
            for (auto iter : suspect_stat) {
                if ((double) iter.second * this->shuffle_size
                    ...
                    uint32_t bid
                        = hid % (uint32_t)(this->shuffle_size * BIN_COUNT);
                    if (this->split_table.find(hid)
                        == this->split_table.end()) {
        ...
        if (recvcount != 0) {
            ...
            MPI_Allgatherv(sendbuf, sendcount, MPI_INT, recvbuf, recvcounts,
                           displs, MPI_INT, this->shuffle_comm);
            for (idx = 0; idx < recvcount; idx++) {
                uint32_t hid = recvbuf[idx];
                uint32_t bid = hid % (uint32_t)(this->shuffle_size * BIN_COUNT);
                this->split_table.insert(hid);
                this->ignore_table.insert(bid);
                PROFILER_RECORD_COUNT(COUNTER_SPLIT_KEYS,
                                      this->split_table.size(), OPMAX);
  ...
  protected:
    void wait()
    {
        ...
        while (iter != this->bin_table.end()) {
            PROFILER_RECORD_COUNT(COUNTER_MAX_BIN_SIZE, iter->second.first,
                                  OPMAX);
            iter++;
        }
    ...
    void save_data()
    {
        ...
        for (k = 0; k < this->shuffle_size; k++) {
            ...
            while (count < recv_count[k]) {
                int kvsize = this->ser->kv_from_bytes(&key, &val, src_buf,
                                                      recv_count[k] - count);
                int ret = this->out->write(key, val);
                if (BALANCE_LOAD && !(this->user_hash) && !(this->ismigrate)) {
                    this->record_bin_info(key, ret);
    ...
    void exchange_kv()
    {
        ...
        MPI_Alltoall(send_offset, 1, MPI_INT, recv_count, 1, MPI_INT,
                     this->shuffle_comm);
        ...
        LOG_PRINT(DBG_COMM,
                  "Comm: exchange KV. (send count=%ld, recv count=%ld, done "
                  "count=%d, peakmem=%ld)\n",
