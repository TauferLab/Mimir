...
void makegraph(MapReduce *mr, char *key, int keybytes, char *value,
...
// columns=(int64_t*)malloc(nlocaledges*sizeof(int64_t));
    ...
    for (int i = 0; i < ncolumn - 1; i++) {
        columns[i] =
            (int64_t *)mem_aligned_malloc(4096, ncolumnedge * sizeof(int64_t));
        if (columns[i] == NULL) {
...
#else
    columns =
        (int64_t *)mem_aligned_malloc(4096, nlocaledges * sizeof(int64_t));
    if (columns == NULL) {
        fprintf(stderr, "Error: allocate buffer for edges (%ld) failed!\n",
                nlocaledges);
        MPI_Abort(MPI_COMM_WORLD, 1);
...
void fileread(MapReduce *mr, char *word, void *ptr)
{
    ...
    mr->add_key_value((char *)&int_v0, sizeof(int64_t), (char *)&int_v1,
                      sizeof(int64_t));
}
...
#endif
            mr->add_key_value((char *)&v1, sizeof(int64_t), (char *)&root,
                              sizeof(int64_t));
        }
    }
}
...
void expand(MapReduce *mr, char *key, int keybytes, char *value, int valuebytes,
            void *ptr)
{
    int64_t v = *(int64_t *)key;
    ...
    if (!TEST_VISITED(v_local, vis)) {
        ...
        for (size_t p = rowstarts[v_local]; p < p_end; p++) {
            ...
            int64_t v1 = columns[p];
#endif
            mr->add_key_value((char *)&v1, sizeof(int64_t), (char *)&v,
                              sizeof(int64_t));
        }
    }
}
...
int64_t getrootvert()
{
    ...
    do {
        ...
        int64_t myroot_proc =
            mypartition((char *)&myroot, (int)sizeof(int64_t));
        if (myroot_proc == rank) {
