...
//a as in argument
uint64_t MapReduce::map(int input_loca, char *in_patha, int read_modea, int map_type, 
		char *log_patha, char *spill_patha, char *out_patha,
		void (*mymap) (MapReduce *, char *, char *, int *, char *, int *, int))
{
	...
	*/
	...
	logf_map_p_of << "Map: enter map function."<<std::endl;
	...
	while (1)
	{
		...
		while (left >= 0)//left = 0 all the data in the input buffer is processed
		{
			if (left > 0)//process elements in input_buffer, omp parallel session
			{
				...
				#pragma omp parallel
				{
					...
					while ((my_done <my_nele) && (!switch_flag))
					{
						...
						if (DEBUG)
						{
							logf_map_t_of<<"Working on key: "<<tmp_key<<", value: "<<tmp_value
								<<std::endl;
						}
						if ((offset_kv+kv_size) < ((LOCAL_BUFFER_SIZE/nprocs) * (proc_id+1)))
						{
							...
							my_done++;
							//for the last couple of kvs that are in local buffer, but local
							//buffer is not full, need to copy to global buffer

							if (DEBUG)
							{
								
								logf_map_t_of <<"If kv fits in local: copy kv "<<tmp_key<<" "<<tmp_value<<" to local buffer. ";
								logf_map_t_of<<"My share is "<<my_nele<<" My done is "
									<<my_done<<" switch flag is "<<switch_flag<<std::endl;

							}
						}else{
							//this kv does not fit in the local buffer, 
							//need to cp local buffer to global buffer
							if (DEBUG)
							{
								
								logf_map_t_of <<"If kv does not fit in local: local kv buffer is full "<<std::endl;
								logf_map_t_of<<"My share is "<<my_nele<<" My done is "
									<<my_done<<" switch flag is "<<switch_flag<<std::endl;
			...
			if (map_type==2){
				...
				exchange_kv(p_map_sendcount1, p_map_recvcount1, p_map_sdispls1, p_map_rdispls1,
					 p_map_sendcount2, p_map_recvcount2, p_map_sdispls2, p_map_rdispls2);
			}else if (map_type==1){
				/**/
			...
			/*determin if all procs are done with map compuataion on this 
			input data buffer*/
			if (left == 0){
...
void MapReduce::wait_previous_ialltoallv_switch_buffer(uint32_t map_num_exchange, 
		bool *switch_flag)
{
	...
	//previously already exchanged kv buffer2, now using kvbuffer 1, need to switch to kvbuffer2
		if (map_num_exchange>1){
			...
			sprintf(p_kv_before_convert,"%s/%s.%d.%d.%d",spill_base.c_str(),"spill_kv_after_all2all_kvb2",
...
uint64_t MapReduce::exchange_kv(int *p_map_sendcount1, int *p_map_recvcount1, int *p_map_sdispls1, 
			int *p_map_rdispls1, int *p_map_sendcount2, int *p_map_recvcount2, 
			int *p_map_sdispls2, int *p_map_rdispls2)
{
	if (DEBUG)
	{
	...
	if (kv_buffer == kv_buffer1)
	{
		...
		MPI_Ialltoallv(kv_buffer_ptr, p_map_sendcount, p_map_sdispls, MPI_BYTE, 
			recvbuf, p_map_recvcount, p_map_rdispls, MPI_BYTE, comm, &map_exchange_request[0]); 
...
uint64_t MapReduce::reduce(int i, uint32_t (*myreduce) (MapReduce *, const char *key, 
			uint32_t keysize, const char *mv, uint32_t mvsize, char *out_kv))
{
	if (DEBUG)
	{
